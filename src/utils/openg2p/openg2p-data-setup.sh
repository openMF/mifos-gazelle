...
ODOO_DB="pbmsdb"
# Fineract clients we mirror into the registry. Matches FINERACT_TENANTS in config.ini.
...
# when $2 (sentinel) is printed. $1 = pod. Mirrors setup-pbms-phee.sh's _odoo_shell.
_odoo_shell() {
...
#------------------------------------------------------------------------------
# Step 1 — fetch clients from Fineract. Emits one TSV line per client with a
# mobile number:  <tenant>\t<mobileNo>\t<displayName>  on stdout.
#------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------
# Step 2 — create registrants in PBMS from the TSV on stdin (built in step 1).
#------------------------------------------------------------------------------
create_registrants() {
...
#------------------------------------------------------------------------------
# Step 3 — create the demo program via OpenG2P's own create-program wizard, which
# sets up ALL the default managers correctly in one transaction (eligibility with
# our age domain, cycle, entitlement, program, and a default payment manager) — the
# same path the "Create Program" UI button runs. Idempotent (created only if absent).
# Enrollment (import_beneficiaries) is intentionally left "no" — a manual task.
#------------------------------------------------------------------------------
create_demo_program() {
  local pod="$1"
  log_step "Creating '${DEMO_PROGRAM_NAME}' + managers (age ${ELIG_MIN_AGE}-${ELIG_MAX_AGE} eligibility, no enrollment)"
...
elig = env["res.partner"].sudo().search_count([
...
#------------------------------------------------------------------------------
# Step 4 — create a PHEE payment manager on the demo program and wire it in.
# Idempotent (created only if the program has no PHEE manager yet). The g2p_payment_phee
# connector only actually uses payment_endpoint_url (multipart CSV POST); the other
# required endpoints/auth fields are set to placeholders. batch_type_header=csv and
# payee_id_type=phone match what setup-pbms-phee.sh configures and the registrant phones.
# create_batch=True is required for "Send Payments" to do anything at all — see the
# comment at the create() call below.
#------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------
# Step 5 — fund the demo program (g2p.program.fund) so it has a funding pool to pay
# entitlements from. Idempotent: skips if a POSTED fund already exists for this
# program (cancelled/draft funds don't count — a posted fund can't be deleted per
# fund_management.py's _unlink_fund, so cancelling is the only way to retire one).
# The exact amount can be tuned by editing PROGRAM_FUND_AMOUNT and re-running after
# cancelling the old fund in the UI — this step does not resize existing funds.
#------------------------------------------------------------------------------
create_program_fund() {
  local pod="$1"
...
main() {
  ...
  log_with_level "$INFO" "Program:  PBMS UI -> Programs -> '${DEMO_PROGRAM_NAME}' (age ${ELIG_MIN_AGE}-${ELIG_MAX_AGE} eligibility). Enroll beneficiaries manually."
